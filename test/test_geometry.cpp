...
TEST_CASE("Size") {
  ...
  SUBCASE("isEmpty with zero width") { CHECK(Size(0, 10).isEmpty()); }
  SUBCASE("isEmpty with zero height") { CHECK(Size(10, 0).isEmpty()); }
  SUBCASE("isEmpty with negative dimensions") { CHECK(Size(-1, 10).isEmpty()); }
  SUBCASE("equality") {
    CHECK(Size(1, 2) == Size(1, 2));
    CHECK_FALSE(Size(1, 2) == Size(3, 4));
  }
  SUBCASE("inequality") { CHECK(Size(1, 2) != Size(3, 4)); }
  SUBCASE("addition") {
