...
TEST_CASE("UTF8 edge: prevPos") {
  SUBCASE("at position 0 returns 0") { CHECK(utf8::prevPos("hello", 0) == 0); }
  ...
  SUBCASE("mixed ASCII and CJK") {
    ...
    // Actually: prevPos(s, 4): p=3. s[3]=0xAD is continuation → p=2. s[2]=0xB8
    // is continuation → p=1. s[1]=0xB8... wait no. s[0]='A', s[1]=0xE4,
    // s[2]=0xB8, s[3]=0xAD, s[4]='B' prevPos(s, 4): start p=3. s[3]=0xAD
    // (10xxxxxx) → p=2. s[2]=0xB8 (10xxxxxx) → p=1. s[1]=0xE4 (1110xxxx) →
    // stop. return 1.
...
TEST_CASE("UTF8 edge: nextPos") {
  SUBCASE("at end returns length") { CHECK(utf8::nextPos("abc", 3) == 3); }

  SUBCASE("past end returns length") { CHECK(utf8::nextPos("abc", 5) == 3); }
