...
TEST_CASE("prevPos") {
  SUBCASE("empty string") { CHECK(prevPos("", 0) == 0); }
  SUBCASE("at position 0") { CHECK(prevPos("hello", 0) == 0); }
...
TEST_CASE("nextPos") {
  SUBCASE("empty string") { CHECK(nextPos("", 0) == 0); }
  SUBCASE("at end") {
    std::string s = "hi";
    CHECK(nextPos(s, 2) == 2);
  }
  ...
  SUBCASE("single byte") { CHECK(nextPos("abc", 1) == 2); }
  SUBCASE("multi-byte") {
