...
void CocoaCanvas::endPaint() {
    if (!backbuffer_ || !nsView_) return;

    NSView* view = (__bridge NSView*)nsView_;
    CGContextRef viewCtx = [[NSGraphicsContext currentContext] CGContext];

    // Note: endPaint is called from drawRect:, so [NSGraphicsContext currentContext]
    // should be valid. If not, we can't draw.
    if (viewCtx) {
        CGImageRef img = CGBitmapContextCreateImage(static_cast<CGContextRef>(backbuffer_));
        if (img) {
            CGContextDrawImage(viewCtx, CGRectMake(0, 0, canvasWidth_, canvasHeight_), img);
            CGImageRelease(img);
        }
    }
}
...
void CocoaCanvas::setFont(const Font& font) {
    ...
    NSFont* nsFont = [NSFont fontWithName:[NSString stringWithFormat:@"%@-%@", family, weightStr]
                                     size:size];
    if (!nsFont) {
...
void CocoaCanvas::drawText(const std::string& text, const Rect& rect, int flags) {
    CGContextRef ctx = static_cast<CGContextRef>(backbuffer_);
    if (!ctx || !currentFont_) return;

    NSFont* font = (__bridge NSFont*)currentFont_;
    NSString* nsStr = [NSString stringWithUTF8String:text.c_str()];
    if (!nsStr || [nsStr length] == 0) return;

    // Create attributed string
    NSDictionary* attrs = @{
        NSFontAttributeName: font,
        NSForegroundColorAttributeName: [NSColor colorWithCalibratedRed:currentR_ / 255.0f
                                                                   green:currentG_ / 255.0f
                                                                    blue:currentB_ / 255.0f
                                                                   alpha:currentA_ / 255.0f]
    };
    NSAttributedString* attrStr = [[NSAttributedString alloc] initWithString:nsStr
                                                                  attributes:attrs];
...
void CocoaCanvas::fillEllipse(const Rect& rect) {
    ...
    CGContextSetRGBFillColor(ctx,
                              currentR_ / 255.0f, currentG_ / 255.0f,
                              currentB_ / 255.0f, currentA_ / 255.0f);
    CGContextFillEllipseInRect(ctx, CGRectMake(rect.x, rect.y, rect.width, rect.height));
}
...
Size CocoaCanvas::measureText(const std::string& text) {
    ...
    return {static_cast<int>(ceil(size.width)), static_cast<int>(ceil(size.height))};
}
