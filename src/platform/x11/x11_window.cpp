...
Size X11Window::getSize() const { return size_; }

void X11Window::invalidate(const Rect &rect) {
  if (window_ && s_display_) {
    if (rect.width > 0 && rect.height > 0) {
      XClearArea(s_display_, window_, rect.x, rect.y, rect.width, rect.height,
                 True);
    } else {
...
NativeCanvas *X11Window::getCanvas() { return canvas_; }
...
float X11Window::dpiScale() const {
  ...
  if (dpiStr && dpiStr[0]) {
    ...
    if (dpi > 0.0f)
      return dpi / 96.0f; // 96 DPI = 1.0 scale factor
  }
...
std::string X11Window::getClipboardText() {
  ...
  // only advances on tick() and would never fire if no frames have been
  // rendered.
  auto deadline =
