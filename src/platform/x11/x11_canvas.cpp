...
void X11Canvas::endPaint() {
  if (!backbuffer_ || !window_)
    return;

  // Blit backbuffer to window
  XCopyArea(display_, backbuffer_, window_, gc_, 0, 0, canvasWidth_,
            canvasHeight_, 0, 0);
  XFlush(display_);
}
...
void X11Canvas::setFont(const Font &font) {
  ...
  std::string pattern =
      font.family + ":pixelsize=" + std::to_string(currentFontDesc_.size);
...
void X11Canvas::fillRect(const Rect &rect) {
  if (!display_ || !backbuffer_ || currentAlpha_ == 0)
    return;
  XFillRectangle(display_, backbuffer_, gc_, rect.x, rect.y, rect.width,
                 rect.height);
}
...
void X11Canvas::strokeRect(const Rect &rect, int lineWidth) {
  ...
  XDrawRectangle(display_, backbuffer_, gc_, rect.x, rect.y, rect.width,
                 rect.height);
...
void X11Canvas::drawText(const std::string &text, const Rect &rect, int flags) {
  ...
  XftDrawStringUtf8(xftDraw_, &xftColor_, xftFont_, tx, ty,
                    reinterpret_cast<const XftChar8 *>(text.c_str()),
                    text.size());
}
...
void X11Canvas::fillEllipse(const Rect &rect) {
  if (!display_ || !backbuffer_ || currentAlpha_ == 0)
    return;
  XFillArc(display_, backbuffer_, gc_, rect.x, rect.y, rect.width, rect.height,
           0, 360 * 64);
}
...
void X11Canvas::strokeEllipse(const Rect &rect, int lineWidth) {
  if (!display_ || !backbuffer_ || currentAlpha_ == 0)
    return;
  XSetLineAttributes(display_, gc_, lineWidth, LineSolid, CapButt, JoinMiter);
  XDrawArc(display_, backbuffer_, gc_, rect.x, rect.y, rect.width, rect.height,
           0, 360 * 64);
  XSetLineAttributes(display_, gc_, 1, LineSolid, CapButt, JoinMiter);
}
...
void X11Canvas::fillRoundedRect(const Rect &rect, int radius) {
  ...
  XFillArc(display_, backbuffer_, gc_, x + w - d, y + h - d, d, d, 270 * 64,
           90 * 64);
...
void X11Canvas::strokeRoundedRect(const Rect &rect, int radius, int lineWidth) {
  ...
  XDrawArc(display_, backbuffer_, gc_, x + w - d, y + h - d, d, d, 270 * 64,
           90 * 64);
