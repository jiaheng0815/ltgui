...
public:
  ...
  bool initialize(void *windowHandle, int width, int height) override {
    ...
    EGLint configAttr[] = {EGL_SURFACE_TYPE,
                           EGL_WINDOW_BIT,
                           EGL_RENDERABLE_TYPE,
                           EGL_OPENGL_ES3_BIT,
                           EGL_RED_SIZE,
    ...
    EGLConfig config;
    EGLint numConfigs;
    if (!eglChooseConfig(display, configAttr, &config, 1, &numConfigs) ||
        numConfigs == 0) {
      // Try ES2 fallback
    ...
    EGLSurface surface = eglCreateWindowSurface(
        display, config, (EGLNativeWindowType)windowHandle, nullptr);
  ...
  void endFrame() override { eglSwapBuffers(display_, surface_); }
  ...
  void destroyTexture(GpuTexture *tex) override {
    delete static_cast<GLTexture *>(tex);
  }
  ...
  void selectShader(int type) override {
    ...
    case 0:
      ...
      break;
    case 1:
      curProg_ = roundedProg_;
      break;
    case 2:
      curProg_ = ellipseProg_;
      break;
    case 3:
      curProg_ = texProg_;
      break;
...
private:
  ...
  void uploadAndDraw(const Vertex2D *verts, int count, GLenum mode) {
    ...
    if (aPos >= 0) {
      ...
      glVertexAttribPointer(aPos, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex2D),
                            (void *)0);
    }
    ...
    if (aCol >= 0) {
      ...
      glVertexAttribPointer(aCol, 4, GL_UNSIGNED_BYTE, GL_TRUE,
                            sizeof(Vertex2D), (void *)16);
    }
    // UV at offset 8
    GLint aUV = glGetAttribLocation(prog, "aUV");
    if (aUV >= 0) {
      glEnableVertexAttribArray(aUV);
      glVertexAttribPointer(aUV, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex2D),
                            (void *)8);
    }
    // Params at offset 20
    ...
    if (aParams >= 0) {
      ...
      glVertexAttribPointer(aParams, 4, GL_FLOAT, GL_FALSE, sizeof(Vertex2D),
                            (void *)20);
    }
    ...
    if (aParams >= 0)
      glDisableVertexAttribArray(aParams);
  }
...
GpuDevice *CreateGLDevice() { return new GLDevice(); }
