  ...
  if (dir) {
    ...
    while ((ent = readdir(dir)) != nullptr) {
      ...
      std::string vendorPath =
          std::string("/sys/class/drm/") + ent->d_name + "/device/vendor";
      ...
      if (vf.is_open()) {
        ...
        else if (info.vendor == GpuVendor::AMD)
          info.name = "AMD GPU";
        else if (info.vendor == GpuVendor::Intel)
          info.name = "Intel GPU";
        ...
        while ((entry = IOIteratorNext(iter)) != 0) {
          ...
          if (classCodeRef) {
            ...
            if (CFGetTypeID(classCodeRef) == CFDataGetTypeID()) {
              ...
              CFNumberGetValue((CFNumberRef)classCodeRef, kCFNumberSInt32Type,
                               &classCode);
            }
            ...
            if ((classCode >> 16) == 0x03) {
              ...
              if (modelRef) {
                if (CFGetTypeID(modelRef) == CFStringGetTypeID()) {
                  char buf[256] = {};
                  CFStringGetCString((CFStringRef)modelRef, buf, sizeof(buf),
              ...
              if (vendorRef) {
                ...
                if (CFGetTypeID(vendorRef) == CFDataGetTypeID()) {
                  ...
                  CFNumberGetValue((CFNumberRef)vendorRef, kCFNumberSInt32Type,
                                   &vendorId);
                }
                CFRelease(vendorRef);
                info.vendor = vendorFromId(vendorId);
  ...
  std::sort(result.begin(), result.end(),
            [](const GpuInfo &a, const GpuInfo &b) {
              if (a.isIntegrated != b.isIntegrated)
                return !a.isIntegrated; // discrete first
