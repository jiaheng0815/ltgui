...
int TextureManager::upload(int w, int h, const uint8_t *rgba) {
  ...
  if (!tex)
    return -1; // GPU texture creation failed — don't store nullptr
  ...
  if (!freeSlots_.empty()) {
    ...
    freeSlots_.pop_back();
    if (textures_[id])
      device_->destroyTexture(textures_[id]);
    textures_[id] = tex;
    return id;
  }
...
Renderer2D::Renderer2D(GpuDevice *device) : device_(device), texMgr_(device) {
  cmds_.reserve(4096);
}
...
void Renderer2D::fillLinearGradient(const Rect &r, const Color &from,
  ...
  cmds_.push_back({DrawOp::FillGradientRect,
                   r,
                   from,
                   0,
                   0,
...
void Renderer2D::strokeRoundedRect(const Rect &r, float radius, float lineWidth,
                                   const Color &c) {
  if (c.a == 0)
    return;
...
void Renderer2D::drawLine(const Point &p1, const Point &p2, float lineWidth,
                          const Color &c) {
  if (c.a == 0)
    return;
...
void Renderer2D::drawGlyph(int texId, const Rect &dst, const Rect &src,
                           const Color &c) {
  DrawCmd cmd;
...
void Renderer2D::flushBatch() {
  ...
  auto shaderFor = [](DrawOp op) -> int {
    ...
    case DrawOp::FillGradientRect:
      return 0; // Solid (per-vertex color)
  ...
  while (idx < cmds_.size()) {
    ...
    verts.reserve((batchEnd - idx) *
                  32); // worst case: StrokeEllipse thick = 192 verts/cmd
    ...
    for (size_t i = idx; i < batchEnd; i++) {
      ...
      case DrawOp::FillRoundedRect:
        emitQuad(verts, cmd.rect, 0, 0, 1, 1, color,
                 ...
                 static_cast<float>(cmd.rect.height), cmd.radius, -1.0f);
        break;
      case DrawOp::FillEllipse:
        emitQuad(verts, cmd.rect, 0, 0, 1, 1, color,
                 static_cast<float>(cmd.rect.width),
                 static_cast<float>(cmd.rect.height), 0, -1.0f);
        break;
      case DrawOp::StrokeRect:
      ...
      case DrawOp::StrokeRounded:
        emitQuad(verts, cmd.rect, 0, 0, 1, 1, color,
                 static_cast<float>(cmd.rect.width),
                 static_cast<float>(cmd.rect.height), cmd.radius,
                 cmd.lineWidth);
        break;
      case DrawOp::StrokeEllipse:
...
static inline float toNdcX(float px, int screenW) {
  return (px / (float)screenW) * 2.0f - 1.0f;
}
static inline float toNdcY(float py, int screenH) {
  return 1.0f - (py / (float)screenH) * 2.0f;
}

void Renderer2D::emitQuad(std::vector<Vertex2D> &out, const Rect &r, float u0,
...
void Renderer2D::emitGradientQuad(std::vector<Vertex2D> &out, const Rect &r,
  ...
  Vertex2D v[] = {
      ...
      {x1, y0, 1, 0, cTR, 0, 0, 0, 0}, {x1, y1, 1, 1, cBR, 0, 0, 0, 0},
  };
  out.insert(out.end(), v, v + 6);
}

void Renderer2D::emitStrokeRect(std::vector<Vertex2D> &out, const Rect &r,
  ...
  auto addTriPair = [&](float ax, float ay, float bx, float by, float cx,
  ...
  // Top band: outer top-left .. outer top-right, inner top-left .. inner
  // top-right
  addTriPair(x0, y0, x1, y0, x0, yi0, x1, yi0);
  // Bottom band: outer bottom-left .. outer bottom-right, inner bottom-left ..
  // inner bottom-right
  addTriPair(x0, yi1, x1, yi1, x0, y1, x1, y1);
  // Left band: outer left .. inner left, spanning from inner-top to
  // inner-bottom
  addTriPair(x0, yi0, xi0, yi0, x0, yi1, xi0, yi1);
  // Right band: inner right .. outer right, spanning from inner-top to
  // inner-bottom
  addTriPair(xi1, yi0, x1, yi0, xi1, yi1, x1, yi1);
}

void Renderer2D::emitStrokeEllipse(std::vector<Vertex2D> &out, const Rect &r,
                                   uint32_t color, float lineWidth) {
...
void Renderer2D::emitLine(std::vector<Vertex2D> &out, const Point &p1,
                          const Point &p2, uint32_t color) {
  Vertex2D v[] = {
...
void Renderer2D::emitThickLine(std::vector<Vertex2D> &out, const Point &p1,
                               const Point &p2, float lineWidth,
                               uint32_t color) {
  ...
  Vertex2D verts[6] = {
      {x0, y0, 0, 0, color, 0, 0, 0, 0}, {x3, y3, 0, 0, color, 0, 0, 0, 0},
      {x1, y1, 0, 0, color, 0, 0, 0, 0}, {x1, y1, 0, 0, color, 0, 0, 0, 0},
      {x3, y3, 0, 0, color, 0, 0, 0, 0}, {x2, y2, 0, 0, color, 0, 0, 0, 0},
  };
