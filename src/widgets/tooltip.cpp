...
void Tooltip::dismiss() { setVisible(false); }

Size Tooltip::sizeHint() const {
  if (!sizeHintDirty())
    return cachedSizeHint();
  setCachedSizeHint(
      textSizeHint({60, 22}, style().paddingHorz(), style().paddingVert()));
  return cachedSizeHint();
}
...
void Tooltip::paintSelf(NativeCanvas *canvas) {
  ...
  canvas->drawText(text_,
                   r.adjusted(style().paddingLeft, style().paddingTop,
...
void Tooltip::show(Widget *target, const std::string &text) {
  ...
  if (parent)
    parent->addChild(std::move(tooltip));
  raw->showAt(pos);
}
