...
std::string SimpleTableModel::cellText(int row, int col) const {
    if (row < 0 || row >= (int)data_.size()) return {};
...
void SimpleTableModel::addRow(const std::vector<std::string>& cells) {
    data_.push_back({});
    auto& row = data_.back();
    for (auto& c : cells) row.push_back(c);
...
void SimpleTableModel::sort(int column, bool ascending) {
    if (column < 0 || column >= colCount_) return;
    std::stable_sort(data_.begin(), data_.end(),
        [column, ascending](const auto& a, const auto& b) {
            if (column >= (int)a.size() || column >= (int)b.size()) return false;
...
int TableView::colX(int idx) const {
    int x = 0;
    for (int i = 0; i < idx; i++) x += columns_[i].width;
    return x;
}
...
int TableView::hitTestCol(int localX) const {
    ...
    for (int i = 0; i < (int)columns_.size(); i++) {
        if (localX >= x && localX < x + columns_[i].width) return i;
        x += columns_[i].width;
    }
    return -1;
}
...
void TableView::paintSelf(NativeCanvas* canvas) {
    ...
    if (visCount <= 0) visCount = std::min(numRows, 20);
    ...
    for (int i = firstVis; i < std::min(numRows, firstVis + visCount); i++) {
        ...
        bool sel = (i == selectedRow_ ||
                    std::find(selectedRows_.begin(), selectedRows_.end(), i) != selectedRows_.end());
        if (sel) {
...
bool TableView::handleEvent(Event& event) {
    ...
    case EventType::MouseDown:
        ...
        for (int c = 0; c < (int)columns_.size() - 1; c++) {
            ...
            if (std::abs(localX - edge) < 4 && columns_[c].resizable) {
                ...
                if (auto* win = window()) win->setCursor(CursorShape::SizeWE);
                event.accepted = true;
                return true;
            }
        }
    ...
    case EventType::MouseMove:
        if (resizingCol_ >= 0) {
            ...
            int newW = std::max(columns_[resizingCol_].minWidth, resizeStartW_ + delta);
    ...
    case EventType::MouseUp:
        if (resizingCol_ >= 0) {
            resizingCol_ = -1;
            if (auto* win = window()) win->setCursor(CursorShape::Arrow);
            event.accepted = true;
            return true;
        }
        break;
    ...
    case EventType::MouseWheel:
        if (!model_) break;
        scrollY_ = std::max(0, std::min(model_->rowCount() - visibleRows(), scrollY_ - event.wheelDelta / 120));
        update();
        event.accepted = true;
        return true;
