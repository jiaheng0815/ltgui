...
Widget *ScrollArea::widget() const { return contentWidget(); }
...
void ScrollArea::scrollTo(int x, int y) {
  ...
  if (auto *cw = contentWidget()) {
    cw->setGeometry(
        Rect(-scrollX_, -currentScrollY(), contentWidth_, contentHeight_));
  }
  update();
}
...
void ScrollArea::paintSelf(NativeCanvas *canvas) {
  ...
  if (contentHeight_ > 0 && contentHeight_ > height()) {
    ...
    int thumbH =
        std::max(24, (int)((int64_t)height() * height() / contentHeight_));
    ...
    canvas->fillRoundedRect(Rect(r.right() - sbWidth, r.y, sbWidth, r.height),
                            4);

    // Theme provides hover/active scrollbar colors (wired up here).
    Color thumbColor =
        draggingScrollbar_ ? t.scrollbarActive : t.scrollbarThumb;
    canvas->setColor(thumbColor);
    canvas->fillRoundedRect(
        Rect(r.right() - sbWidth + 2, thumbY + 1, sbWidth - 4, thumbH - 2), 3);
  }
}
...
bool ScrollArea::handleEvent(Event &event) {
  ...
  bool onScrollbar = (contentHeight_ > 0 && contentHeight_ > height()) &&
                     (localX >= width() - 12);

  if (event.type == EventType::MouseDown && onScrollbar) {
    // Calculate scrollbar thumb position (contentHeight_ > 0 guaranteed by
    // onScrollbar)
    int curScrollY = currentScrollY();
    int thumbH =
        std::max(24, (int)((int64_t)height() * height() / contentHeight_));
  ...
  if (auto *cw = contentWidget()) {
    if (!onScrollbar) {
      ...
      bool inContent =
          cx >= 0 && cy >= 0 && cx < contentWidth_ && cy < contentHeight_;
