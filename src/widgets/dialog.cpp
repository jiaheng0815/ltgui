...
Size Dialog::sizeHint() const { return {panelW_, panelH_}; }

void Dialog::addButton(const std::string &text, DialogResult res,
                       bool isDefault) {
  if (!panel_)
    return;
...
MessageBox::MessageBox(Widget *parent) : Dialog(parent) { rebuild(); }
...
DialogResult MessageBox::show(Widget *parent, const std::string &title,
                              const std::string &message, int buttons,
                              Icon icon) {
  (void)icon;
...
void MessageBox::paintSelf(NativeCanvas *canvas) { Dialog::paintSelf(canvas); }

// --- InputDialog ---

InputDialog::InputDialog(Widget *parent) : Dialog(parent) { rebuild(); }
...
std::string InputDialog::text() const { return input_ ? input_->text() : ""; }
