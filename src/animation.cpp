...
AnimatedFloat::AnimatedFloat(float value) : current_(value), target_(value) {}
...
void AnimatedFloat::setTarget(float v, int durationMs, Easing e) {
  ...
  if (target_ == v && durationMs_ == durationMs && easing_ == e && animating_)
    return;
...
WidgetAnimation::WidgetAnimation(WidgetAnimation &&other) noexcept
    : onFinished(std::move(other.onFinished)), anim_(std::move(other.anim_)),
      durationMs_(other.durationMs_), delayMs_(other.delayMs_),
      easing_(other.easing_), loop_(other.loop_), yoyo_(other.yoyo_),
      playing_(other.playing_), startTickMs_(other.startTickMs_),
      startVal_(other.startVal_), endVal_(other.endVal_),
      onValue_(std::move(other.onValue_)), delayPhase_(other.delayPhase_) {
...
KeyframeAnimation::KeyframeAnimation(KeyframeAnimation &&other) noexcept
    : onFinished(std::move(other.onFinished)),
      keyframes_(std::move(other.keyframes_)), durationMs_(other.durationMs_),
      loop_(other.loop_), playing_(other.playing_),
      startTickMs_(other.startTickMs_), onValue_(std::move(other.onValue_)) {
  if (playing_) {
...
void AnimationManager::tick() {
  ...
  for (size_t i = 0; i < widgetAnims_.size();) {
    ...
    // via ~WidgetAnimation), anim is now dangling. Verify liveness.
    {
      ...
      if (it == widgetAnims_.end())
        continue; // destroyed; vector shifted
    }
