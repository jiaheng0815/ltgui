...
void BoxLayout::addStretch(int factor) { stretchFactors_.push_back(factor); }
...
void BoxLayout::setMargin(int margin) { margin_ = margin; }

void BoxLayout::setDirection(Direction dir) { direction_ = dir; }
...
void BoxLayout::layout(Widget *container) {
  ...
  // valid child pointers for an O(n+m) purge instead of the O(n*m) loop.
  {
    ...
    for (auto it = widgetStretch_.begin(); it != widgetStretch_.end();) {
      if (valid.count(it->first))
        ++it;
      else
        it = widgetStretch_.erase(it);
    }
  }
  ...
  int remaining =
      (direction_ == LeftToRight ? availW : availH) - totalHint - totalSpacing;
  if (remaining < 0)
    remaining = 0;
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (direction_ == LeftToRight) {
      ...
      if (totalStretch > 0 && stretch > 0) {
        childW += static_cast<int>(static_cast<int64_t>(remaining) * stretch /
                                   totalStretch);
      }
      childH = availH;
      ...
      childH = hint.height;
      if (totalStretch > 0 && stretch > 0) {
        childH += static_cast<int>(static_cast<int64_t>(remaining) * stretch /
                                   totalStretch);
      }
...
GridLayout::GridLayout(int cols, int rowSpacing, int colSpacing, int margin)
    : cols_(cols), rowSpacing_(rowSpacing), colSpacing_(colSpacing),
      margin_(margin) {}
...
void GridLayout::layout(Widget *container) {
  ...
  for (int w : colWidths)
    usedColW += w;
  int64_t extraW = std::max<int64_t>(0, static_cast<int64_t>(availW) -
                                            totalColSpacing - usedColW);
  int totalColStretch = 0;
  for (int c = 0; c < cols_; c++) {
    auto it = colStretch_.find(c);
    if (it != colStretch_.end())
      totalColStretch += it->second;
  }
  ...
  for (int h : rowHeights)
    usedRowH += h;
  int64_t extraH = std::max<int64_t>(0, static_cast<int64_t>(availH) -
                                            totalRowSpacing - usedRowH);
  int totalRowStretch = 0;
  for (int r = 0; r < rows; r++) {
    auto it = rowStretch_.find(r);
    if (it != rowStretch_.end())
      totalRowStretch += it->second;
  }
