...
void DragData::setFiles(const std::vector<std::string> &paths) {
  ...
  setData("text/uri-list",
          std::vector<uint8_t>(uriList.begin(), uriList.end()));
}
...
std::vector<std::string> DragData::files() const {
  ...
  while (pos < uriList.size()) {
    ...
    if (uri.compare(0, 7, "file://") == 0) {
      ...
      if (uri.compare(0, 10, "localhost/") == 0 ||
          uri.compare(0, 10, "localhost") == 0) {
    ...
    pos = end;
    // Skip the separator
    if (pos < uriList.size() && uriList[pos] == '\r')
      pos++;
...
void DropTarget::handleDrop(const DragData &data) { onDrop.emit(data); }

bool DropTarget::handleDragOver(const DragData &data) {
  if (dragOverCb_)
    return dragOverCb_(data);
  return true;
}
