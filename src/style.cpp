...
Style Style::defaultStyle() { return fromTheme(currentTheme()); }
...
ResolvedStyle Style::resolve(WidgetState state, const Theme &theme) const {
  ...
  out.bgColor =
      pick(bgColor, state == WidgetState::Disabled ? theme.bgTertiary
                                                   : theme.bgSecondary);
  out.fgColor =
      pick(fgColor, state == WidgetState::Disabled ? theme.textDisabled
                                                   : theme.textPrimary);
  ...
  if (accent != Color::Transparent) {
    ...
    case WidgetState::Hovered:
      out.accent = theme.accentHover;
      break;
    case WidgetState::Pressed:
      out.accent = theme.accentPressed;
      break;
    default:
  ...
  if (patch) {
    ...
    if (patch->fgColor)
      out.fgColor = *patch->fgColor;
    if (patch->borderColor)
      out.borderColor = *patch->borderColor;
    if (patch->accent)
