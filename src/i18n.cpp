...
int PluralRules::formIndex(const Locale &locale, int64_t n) {
  ...
  // Polish — One: n == 1; Few: n % 10 in 2..4 && n % 100 not in 12..14; Many: n
  // != 1 && n % 10 in 0..1 or n%10 in 5..9 or n%100 in 12..14
  if (lang == "pl") {
    int m10 = n % 10, m100 = n % 100;
    if (n == 1)
      return One;
    if (m10 >= 2 && m10 <= 4 && (m100 < 12 || m100 > 14))
      return Few;
    if ((n != 1 && (m10 <= 1 || m10 >= 5)) || (m100 >= 12 && m100 <= 14))
      return Many;
    return Other;
  }
...
std::string TranslationTable::getPlural(const std::string &key,
                                        int64_t n) const {
  ...
  if (form != PluralRules::Other) {
    ...
    if (oit != otherMap.end() && !oit->second.empty())
      return oit->second;
  }
...
std::string I18n::tr(const std::string &key, int64_t n) const {
  auto it = tables_.find(locale_.toString());
  if (it != tables_.end())
    return it->second.getPlural(key, n);
...
bool I18n::loadTableFromFile(const Locale &locale,
                             const std::string &jsonPath) {
  TranslationTable table;
  if (!table.loadFromJsonFile(jsonPath))
    return false;
